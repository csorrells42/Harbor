...
export function createDeliverySettings({dataDir,portableRoot,getSettings,updateSettings,retainReplaced=()=>false,reserveCreated=()=>()=>{}}){
  ...
  return input=>{
    const job=queue.then(async()=>{
      ...
      const before=getSettings(),next={...before},created=[],reservations=[];
      ...
      try{
        for(const prefix of ['portkeyApi','portkeyWorkers']){
          ...
          reservations.push(reserveCreated([file]));
        ...
        for(const prefix of ['portkeyApi','portkeyWorkers']){const old=before[prefix+'KeyFile'];if(old&&old!==next[prefix+'KeyFile']&&!retainReplaced(old)&&managed(path.normalize(resolve(old))))await fs.unlink(resolve(old)).catch(()=>{});}
        return result;
