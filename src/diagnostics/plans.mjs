...
export const TASK_IDS=['lookup','chain','recovery','boundary','no-tool','unavailable'];
export const hash=value=>createHash('sha256').update(JSON.stringify(value)).digest('hex');
export const DEFAULT_PLAN=Object.freeze({repetitions:3, seed:18431, maxTrialSeconds:120, maxCampaignSeconds:1800, maxTurns:12, maxHostUsedGiB:null, maxGpuUsedMiB:null, tasks:TASK_IDS, variants:[{id:'all',toolMode:'all'},{id:'bm25',toolMode:'bm25'},{id:'regex',toolMode:'regex'},{id:'code',toolMode:'code'},{id:'semantic',toolMode:'portkey-local'},{id:'hybrid',toolMode:'hybrid',hybridModes:['bm25','portkey-local']}]});
export function validatePlan(input={}) {
  ...
  const allowed=new Set(['id','toolMode','hybridModes','searchLimit','semanticMinScore','portkeyLocalModel']);
  p.variants=p.variants.map(v=>{
    ...
    const settings=validateSettings({...DEFAULT_SETTINGS,...v});
