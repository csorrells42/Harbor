...
test('native Diagnostics runs all six task kinds and reloads independently graded results',{
  ...
  const model='diagnostics-desktop-protocol-fixture',requests=[],steps=new Map(),serverErrors=[];
  const server=createServer(async(req,res)=>{
    try{
      ...
      requests.push({task,step,tool:action?.[0]??null,stream:!!request.stream});
      if(request.stream){
  ...
  try{
    ...
    await page.getByLabel('Seconds per trial (including Hermes startup)',{exact:true}).fill('45');
    ...
    if(process.env.HARBOR_DIAGNOSTICS_CAMPAIGN_EVIDENCE){
      ...
      await page.getByRole('heading',{name:'Recent trials',exact:true}).scrollIntoViewIfNeeded();
