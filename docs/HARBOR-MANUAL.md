...
- Normal application startup does not install a model or start Hermes. Diagnostics trials require a separately available supported Hermes or OpenClaw installation and model server. A GPU is not a prerequisite for the gateway; NVIDIA readings require an accessible `nvidia-smi` and other thermal sensors depend on supported providers.
...
| **Diagnostics** | Live system gauges, temperature history, selected harness connection state, campaign controls, Harbor configuration comparisons and saved trials. | Inspect hardware; choose Hermes or OpenClaw, tasks and budgets; check the selected harness, **Start campaign**, **Cancel campaign**, **Copy results JSON**, **Save campaign evidence**; browse **Saved campaigns**. |
...
### Named client profiles (Phase 2 source implementation)

Open **Profiles** to select a preset or create a named toolbox. Choose its servers, capability types, delivery settings and process ownership, then save. The default endpoint keeps its existing settings; named profiles use `/mcp/profiles/<id>` under the configured MCP path. All profiles share the gateway authentication policy. Profile selection limits discovery and invocation but does not create separately authorized users.

**Shared** profiles use existing server processes. **Separate native process per client session** creates owned native stdio children for each client; configured files, accounts and remote services can still be shared. Existing sessions retain the profile revision they connected with. Reconnect to adopt edits, or use **Disconnect profile sessions**. **Copy configuration** includes the saved gateway key; the visible preview hides it. **Test connection** initializes a real MCP client and lists tools, then closes the session.

Changing a profile's provider key preserves the previous generated file while a saved setting or an initializing, connected or retiring session needs it. Harbor removes unreferenced generated profile keys after lifecycle changes and at startup. It preserves external key files, skips links/junctions and does not recursively remove credential directories. See [profiles and session ownership](profiles.md) for the exact cleanup scope and concurrency limits. Native Profiles acceptance covers key rotation, last-session cleanup, restart and deletion. LM Studio application and final packaged acceptance remain pending.
...
The Phase 2 source adds separate **Enabled for use**, **Start automatically when Harbor opens**, and **Eligible for on-demand start** choices in Configure. Legacy/imported servers keep on-demand activation off. Start a server once to capture its catalog; after Harbor restarts, eligible stopped servers remain discoverable without starting a process. Invocation shares one startup attempt, obtains the live contract and validates arguments before dispatch. A changed contract requires refreshed discovery. Search alone never installs or starts an upstream.

The idle policy applies only to processes started on demand, after calls finish and no named shared profile session retains the server. It preserves manual/startup ownership. **Stop** temporarily suppresses on-demand use; disabling the server prevents use across launches. The server card shows cached-tool count and activation state. See [on-demand activation](on-demand.md) for cache invalidation, failure/cancellation behavior and HTTP/SSE ownership limits. Final packaged acceptance remains pending.
...
Advisor includes an offline toolbox-rules panel and a separate **Measured configuration advice** panel. Toolbox rules use configured server IDs and runtime snapshots; they do not call an AI service, search the internet, install anything or execute tools. Unrecognized custom server IDs are labeled optional with unknown capability/network assumptions.

**Measured configuration advice** reads saved local Diagnostics evidence. Choose **Load saved evidence**, check a campaign and inspect its task coverage, uncertainty, adherence, latency and resource tradeoffs. Incomplete, stale or contradictory evidence withholds a change. Supported advice offers a before/after review and an explicit Apply action for a named profile; existing sessions retain their revision until reconnecting. Optional history is off by default and retains changed delivery values only, without application backups. See [measured advice](measured-advice.md) for eligibility, retention and current acceptance limits.
...
Harbor's Phase 2 source provides a **Streamable HTTP MCP gateway for tools, resources and prompts**. The desktop controls use local Electron IPC. Child-server tool/prompt names and resource references are namespaced to avoid collisions; use the exact names and URIs advertised by Harbor. Resources include listing, reading, templates and subscriptions; prompts include listing and retrieval. Profiles apply to every primitive type. **Test connection** reports the actual enabled catalogs, including partial discovery failures. Sampling, elicitation, roots, completion and task forwarding are unsupported. See the [capability matrix and troubleshooting details](mcp-capabilities.md) for negotiation, limits, cancellation and subscription ownership. These are source capabilities; final Phase 2 Portable/client acceptance remains pending.

#### A client on the same computer: Streamable HTTP

1. In **Help with connections**, choose **Local** as the configuration address.
...
In **Connections → Help with connections**, choose **Client application**, then the address and transport. The same choices are available when connecting a named profile. **Copy configuration** uses the selected client's format and includes the saved Harbor key when required; the preview only shows a placeholder. Merge the Harbor entry into the client's existing configuration rather than replacing the whole file.

**Hermes:** choose **Hermes** to generate YAML for `mcp_servers.harbor`. Open `config.yaml` in the home or profile used by your Hermes client. The upstream CLI default is `~/.hermes/config.yaml`; Hermes Desktop installations and `HERMES_HOME` overrides can use another directory. Merge the entry under the existing `mcp_servers` section with spaces for indentation, save, restart the client, and inspect its available tools. HTTP exports use `url` and optional `headers.Authorization`; stdio exports use `command`, `args`, and optional `env.HARBOR_API_KEY`. [Hermes MCP documentation](https://hermes-agent.nousresearch.com/docs/user-guide/features/mcp)

**OpenClaw:** choose **OpenClaw** to generate JSON for `mcp.servers.harbor`. This recipe targets native MCP support, verified against the installed OpenClaw **2026.7.1-2** documentation. Merge it into your active configuration, normally `~/.openclaw/openclaw.json`. It is not a `mcporter.json` recipe. Streamable HTTP explicitly sets `transport: "streamable-http"`; stdio sets `transport: "stdio"`. Save, run `openclaw mcp doctor harbor --probe`, then reconnect your agent. That probe connects to Harbor and checks discovery; it does not prove the model can complete a task. The agent's tool policy must permit MCP tools; the minimal tool profile hides them. [OpenClaw MCP documentation](https://docs.openclaw.ai/cli/mcp)

For a client on another computer, use a reachable network address. `127.0.0.1` refers to the computer running the client. With the stdio bridge, Node.js and the bridge file must exist at the exported paths on that client computer; Streamable HTTP avoids that extra installation.

#### More client recipes

The **Client application** dropdown also includes OpenCode v2, OpenCode v1, OpenHands, Goose, Open Interpreter, Open WebUI, Letta, AnythingLLM and **General / another MCP client**. Choose your program, address and transport. The connection string, numbered steps and copyable configuration update together. Open WebUI automatically selects Streamable HTTP and hides stdio, which its native MCP integration does not support. Named profiles export their own endpoint, including `/profiles/<id>`.

These are setup recipes checked against the sources linked below. Recipe and clipboard tests verify Harbor's export; they do not establish that every external application/version has passed a live connection and tool call. Client-side tool policies and model tool-use ability still apply. Harbor serves tools, resources and prompts where the client supports them; sampling and elicitation are unsupported.

| Program | Where the copied values go | Important difference |
| --- | --- | --- |
| OpenCode v2 | Active `opencode.jsonc`, under `mcp.servers.harbor` | Remote uses `type: remote`, `url`, optional headers and `oauth: false`; local uses a command array. |
| OpenCode v1 | Active `opencode.json`, under `mcp.harbor` | Choose the v1 recipe; its wrapper differs from v2. |
| OpenHands | Settings → MCP; or `[mcp]` in `config.toml` for deployments using file configuration | Append to `shttp_servers` or `stdio_servers`. A container must reach the selected URL or have the bridge paths available inside it. |
| Goose | Settings → Extensions → Add custom extension, or `goose configure`; YAML goes under `extensions.harbor` | HTTP uses `streamable_http` and `uri`. Stdio uses `cmd`, `args` and `envs`. Enable the extension. |
| Open Interpreter | `~/.openinterpreter/config.toml`, under `[mcp_servers.harbor]` | Requires a version with `interpreter mcp` support. HTTP uses `http_headers`; `/mcp` checks loaded servers. |
| Open WebUI | Admin → Integrations → External Tool Servers → Add Connection | Choose **MCP (Streamable HTTP)**. Copy gives form values, not an OpenAPI import. Enter the token without `Bearer` in the token field. |
| Letta | MCP server management, or the body of `POST /v1/mcp-servers/` on your Letta server | The request contains `server_name` and `config`; Harbor's `custom_headers` are separate from Letta account authentication. Attach discovered tools to the agent. |
| AnythingLLM | `plugins/anythingllm_mcp_servers.json` in its storage directory | Retain `type: streamable` for HTTP. Reload MCP servers, check Agent Skills and use `@agent`. |
| General | Your client's Add MCP server screen or configuration | Uses the common `mcpServers` wrapper. Adapt that wrapper if required; this endpoint is not an OpenAI model URL or legacy SSE endpoint. |

For HTTP, the selected URL must be reachable from the actual client backend. In Docker, `localhost` means the container. For stdio, the client must have Node.js and the bridge at the displayed paths. Changing the address in Harbor's dropdown does not configure routing, firewall rules or TLS. Keep Harbor running and use **Copy configuration** to include its saved key. The displayed preview deliberately hides the key.

Source references for these recipes: [OpenCode v2](https://opencode.ai/v2/docs/mcp-servers), [OpenCode v1](https://opencode.ai/docs/mcp-servers), [OpenHands](https://docs.openhands.dev/openhands/usage/settings/mcp-settings), [Goose configuration](https://github.com/aaif-goose/goose/blob/main/documentation/docs/guides/config-files.md), [Open Interpreter](https://www.openinterpreter.com/docs/terminal/mcp), [Open WebUI](https://docs.openwebui.com/features/extensibility/mcp/), [Letta API](https://docs.letta.com/api/python/resources/mcp_servers/methods/create), [AnythingLLM](https://docs.anythingllm.com/mcp-compatibility/overview), and [MCP remote connections](https://modelcontextprotocol.io/docs/develop/connect-remote-servers).

#### A client that only supports stdio

Select **Stdio bridge** in Harbor's **Help with connections**, then copy that configuration. The generated command uses the bundled `runtimes/node/node.exe` in Portable and the active release's `resources/bridge.mjs`. The bridge forwards JSON-RPC over HTTP to the existing Harbor gateway. It does not launch another copy of every child server.
...
5. Under **Active connection**, copy an advertised LAN endpoint. In **Help with connections**, select that LAN address before copying the client configuration.
...
5. Choose **Copy key** to copy the **saved** key, or use **Help with connections → Copy configuration** to copy the current endpoint and saved credential together.
...
Choose **Hermes**, **OpenClaw** or **LM Studio** in **1. Connection > Test harness**. The selection controls the actual trial runner and is saved with the campaign. A campaign uses one harness and one already-running model. Changing harnesses requires a new connection check; the selector is unavailable during an active campaign. None of these adapters issues model lifecycle requests.

The first adapter targets the installed Windows Hermes agent and its configured local llama.cpp or LM Studio model. The default adapter looks under `%LOCALAPPDATA%\hermes` and uses its `hermes-agent\venv\Scripts\python.exe`. It reads the Hermes configuration and existing local model-server state; it does not start, stop, download or switch the model. Start the intended model in Hermes or load it explicitly in LM Studio first. For LM Studio, exactly one language-model instance must be loaded, with the configured context length and single-request concurrency. A saved model key is resolved to that already-loaded instance; a missing or different model fails preflight.

**Check Hermes** verifies the supported installed revision, selected source/runtime fingerprints, the recorded model-server PID and the loopback model catalog. The exact compatibility revision is currently `1675f1f2c25ce164f07c42e829f2c17a723db94f`; a different revision fails closed pending adapter revalidation. A saved configuration alone is not a ready result. The model name must be advertised by the active catalog. Process ownership, start time, runtime versions and observed request settings are retained and checked again after each trial. Supported llama.cpp builds can attest weight files and live server properties; LM Studio currently records native loaded-instance metadata and load settings without complete weight/runtime/default attestation. Missing evidence prevents recommendations, including provisional leaders. See [setup identity and eligibility](diagnostic-workloads.md#setup-identity-and-recommendation-eligibility) and [model provenance](model-identity.md).

For LM Studio toggle models, a saved reasoning effort such as `high` may be outside the advertised `off`/`on` choices. The normal diagnostic follows Hermes by omitting that request field and leaving the server's declared default in control. Evidence distinguishes the saved request, omitted field and published default; it does not claim the model supports that effort. Explicit benchmark settings reject unsupported efforts rather than substituting controls. The user's saved settings are unchanged.
...
#### OpenClaw setup and differences

The OpenClaw adapter targets installed version **2026.7.1-2**, normally under `%APPDATA%\npm\node_modules\openclaw`. It runs the real embedded agent in a fresh state directory for each trial. It requires exactly one loaded LM Studio language model at `http://127.0.0.1:1234/v1`, a verified context of at least 4,000 tokens, and one concurrent prediction. Load and configure the model in LM Studio first. **Check OpenClaw** verifies the installed entry point and compiled runtime fingerprint plus the loaded instance/settings. Different OpenClaw versions require adapter verification. Authentication-enabled model endpoints are not currently supported by this adapter.

Trials use only their private Harbor MCP connection. Ordinary OpenClaw channels, sessions, credentials, bundled skills, memory and built-in tools are not imported. A local relay captures the actual outbound tool definitions and inference controls, checks loaded-model settings before each request, and rejects model lifecycle routes, model switches and concurrent predictions. It does not copy model messages into ordinary diagnostic request evidence. Tool dispatch is observed through a trial-local OpenClaw plugin. The independent task verifiers remain the same as for Hermes.

OpenClaw uses a 1,024-token output reserve, no additional compaction reserve floor, and at most 2,048 recent-history tokens. These isolated harness settings let an 8K model run the integration tasks without changing its LM Studio context. Hermes and OpenClaw have different prompt and inference policies; their results must retain that distinction. Both adapters apply the explicit final JSON response contract to tasks that declare it.

OpenClaw reports host RAM/GPU samples; its worker RSS and host CPU utilization are currently unknown. Timing includes installed-agent startup and observation overhead. Compiled entry/runtime files are fingerprinted, but transitive dependency bytes, LM Studio weight bytes, complete server defaults and endpoint process ownership are not attested. Missing identity withholds measured recommendations. Read [OpenClaw diagnostics](openclaw-diagnostics.md) for the current acceptance scope. A successful integration fixture proves runner behavior, not the reasoning quality of a model.

#### Native LM Studio setup and differences

Choose **LM Studio** to evaluate Harbor tool use through the native LM Studio agent loop, separately from Hermes or OpenClaw using the same model server. It requires LM Studio 0.4.0 or newer, the local server at `http://127.0.0.1:1234`, exactly one loaded language model, at least 4,000 context tokens and one concurrent prediction. On tested LM Studio 0.4.21+2, enable **Require Authentication**, then **Allow calling servers from mcp.json** in Server Settings. Create a token with that same configured-server permission and supply it as `LM_STUDIO_API_TOKEN` in Harbor's launch environment; exports do not include the token. Authentication also affects other clients connected to this server. Checking verifies inventory; a trial verifies the native MCP connection.

Each trial temporarily adds its own uniquely named configured MCP entry, then removes that exact entry while preserving existing servers. The native API rejects private localhost addresses for per-request MCPs, so that separate option is insufficient. A fresh, unsaved chat selects only the trial's MCP and uses temperature 0.2 with a 1,024-token output limit. Existing model load/context/reasoning settings are not changed. The same conformance and representative task graders apply. See [native LM Studio diagnostics](lmstudio-diagnostics.md) for setup, cleanup and acceptance status.

Harbor waits up to ten seconds for LM Studio's configuration synchronization receipt before submitting the trial. This integration is verified with 0.4.21+2; missing receipt support fails setup without inference. Anonymous tool-start boundaries are not counted as attributed calls. Complete argument/failure events establish attempts, successful results must identify the trial's own plugin, and interleaved calls are reconciled before grading. A real native tiny-model trial read the correct record but omitted the required write; Harbor marked it incomplete despite its completion claim.

Tool names, argument validity, invalid calls, final responses and Harbor-side effects are observable. Exact internal per-turn tool presentation, complete inference settings and model provenance remain unknown, withholding measured recommendations. Native result grades do not establish that a model will behave identically in Hermes or OpenClaw. After an interrupted native stream, the campaign stops because LM Studio generation cancellation cannot be confirmed; verify the model is idle before restarting. A hard crash can leave an inactive `harbor-diagnostic-<id>` configuration entry requiring narrow manual removal. Live installed acceptance is pending the configured-MCP server permission; deterministic protocol tests are not a substitute for that check.

#### Run a campaign

1. In **1. Connection**, select **Test harness**, then choose **Check Hermes**, **Check OpenClaw** or **Check LM Studio**. Resolve any probe failure before interpreting campaign results. **Connection details** explains the observed model identity.
2. Continue to **2. Tasks** and select the conformance or representative pack. Expand **Choose individual tasks** to adjust the selection.
3. Continue to **3. Delivery** and select the methods to compare: All tools, BM25, regex, Code Mode, local semantic search or Hybrid. **Advanced delivery settings** holds search tuning, Hybrid methods, controlled catalog comparisons and the custom JSON matrix. Choose at least two distinct methods for Hybrid.
4. Continue to **4. Limits** for repetitions and trial/campaign durations. **Advanced limits and repeatability** holds the matched seed, model-turn limit and optional memory ceilings. In **5. Review and run**, check the task count, methods, total trials and time limits, then choose **Start campaign**. Step headings and the navigation buttons let you return without losing selections. Trials execute sequentially in seeded, balanced randomized order; a fresh conversation/fixture does not imply a cold model load.
5. Use **Cancel campaign** to request cancellation and prevent subsequent trials. For native LM Studio, remote generation cancellation remains unconfirmed after interruption. Closing the tab does not cancel a campaign. Application exit requests cancellation and cleanup.
6. Review **Harbor configurations**, open the saved campaign settings and inspect individual trials. **Copy results JSON** copies a compact summary; **Save campaign evidence** saves full recorded trial details through a native dialog. **Saved campaigns** opens earlier runs. The interface shows the local result-directory location.

![Guided delivery choices with advanced settings collapsed](images/screenshots/05a-diagnostics-delivery.png)

Continue opens and scrolls to the next step. Step headings and the numbered navigation return to earlier choices without resetting them. Hardware monitoring stays available below the results without crowding the setup path.

![Native campaign review with ten representative tasks and six delivery methods selected](images/screenshots/05-diagnostics-campaign.png)

These screenshots show the actual native application in a disposable profile. No model probe or campaign was run for the screenshots. The review shows 180 planned trials: ten tasks, six methods and three repetitions. A displayed trial plan is not a benchmark result. Use **Check Hermes** before starting; scores and comparisons require a compatible, running harness and model.
...
The normal GUI preserves the six conformance tasks as the default selection and adds ten representative tasks. Six variants × six default tasks × three repetitions gives 108 planned trials, subject to the campaign time limit. See [representative workloads](diagnostic-workloads.md) for task oracles, real SQLite/Chromium prerequisites and evidence limits. Memory ceilings are sampled stop conditions, not OS reservations or hard allocation caps. Host/GPU use includes the existing model server and competing workloads. Brief peaks between samples may be missed. No CPU affinity, model VRAM allocator or thermal/power-control mechanism is provided.
...
Allowed variant fields are `id`, `toolMode`, `hybridModes`, `searchLimit`, `semanticMinScore`, `portkeyLocalModel` and `catalog`. The ordinary **Controlled catalog comparison** controls add a paired candidate for each selected mode, with complete-workflow curation, concise descriptions, schema annotation reduction, catalog ordering and fixture-provider preference. See [controlled catalog comparisons](diagnostic-workloads.md#controlled-catalog-comparisons) for their exact boundaries, including observed Hermes reordering. This adapter rejects hosted embedding variants. A different configured model or harness requires a separate supported pairing; the current GUI is not a model-manager interface.
...
| --- | --- |
| Verified completion | Required fixture state, plus exact sequences for conformance tasks or independent artifact checks for representative tasks. Abstention tasks enforce their specific no-action/response contract. A harness completion flag alone is insufficient. |
...
| Tool selection correctness | Within each trial, schema-valid successful/expected-transient fixture calls matching the conformance sequence or the representative task's allowed capability set, divided by observed fixture calls. The table averages known trial ratios; discovery-only calls do not count as correct fixture actions. No fixture calls generally leaves this unknown. |
| Argument correctness | Schema-valid calls divided by calls with a known schema result at the harness execution callback, including discovery wrappers. The table averages known trial ratios. Calls rejected before observation are not fully observable; unknown schema results are not counted as invalid arguments. |
...
| Resources | Sampled system RAM/GPU use and, when available, host CPU utilization and worker RSS. OpenClaw currently leaves worker RSS and CPU unknown. The resource sample cadence is about two seconds plus probe time. This does not isolate the model process tree's exclusive consumption. |
...
Measured comparisons also require [verified model identity](model-identity.md). **Check Hermes** reads the selected running model's effective context/defaults and hashes its GGUF weights plus runtime bundle. It reports missing evidence explicitly. The probe uses the validated local runtime's no-autoload request and never starts or downloads a model. Large-file hashing has a two-minute probe budget; trial elapsed time includes its own verification/startup work. A configured model name or successful API response alone cannot establish a comparable setup. Live LoRA state and runtime builds outside the validated adapter gate remain unverified.

The conformance pack retains exact sequence requirements, so a different valid approach can fail that rubric. The representative pack checks artifacts and accepts defined alternative workflows. Success on these small synthetic tasks does not establish performance on long coding jobs, general user usefulness or acceptance by a human.

#### Harbor delivery comparisons and history

**Harbor configurations** compares delivery variants within one fixed setup. Each trial records selected Harbor settings, relevant source fingerprint, harness revision/source fingerprint, configured model/provider/reasoning, hardware identity, suite version, task seed, catalog identity and repetition. Models and harnesses are setup identities, not competitive views.

An always-on emergency floor also stops a campaign when available system memory reaches 500 MB (500,000,000 bytes) or less. Harbor checks before starting a trial and every half second while it runs, independently of model telemetry. This does not change the more cautious configured RAM/GPU limits. If a configured RAM or GPU limit is exceeded, Harbor stops the current trial, cleans up its owned resources, and ends the entire campaign. Remaining trials stay unrun. The limit uses sampled system-wide usage, including other programs; it is not a reservation or a guarantee against peaks between samples.

Each configuration also shows excluded, timed-out, cancelled, infrastructure-failed and resource-stopped counts against its recorded trials. Exclusions and status counts may overlap. Tool-selection and argument-observation counts use eligible trials as their denominator; a missing observation is unknown, not a zero score. The campaign's recorded/planned count separately exposes unfinished coverage.

The grader combines observed actions, synthetic fixture state and the required response; a harness's own completion claim is recorded separately. Eligibility checks govern quality rates, while excluded trials remain visible. Representative tasks accept defined alternative workflows; the conformance pack retains strict sequence checks.

Each campaign observes one selected Hermes/model or OpenClaw/model pairing. The latest campaign is shown after restart. **Saved campaigns** browses older runs without aggregating unmatched evidence. Its comparison, settings, trial inspection and copy/save actions refer to the selected snapshot. **Show latest campaign** returns to the current run. Local export/read limits and troubleshooting are documented in [workloads and evidence](diagnostic-workloads.md). This manual and its regenerated PDF reflect Phase 2 source development. Earlier screenshots illustrate the baseline; final packaged release acceptance remains pending.
...
2. Open **Maintenance** and select **Enter maintenance mode**. While **Stopping servers…** is shown, component updates remain unavailable. They become available only after delivery workers and owned upstream processes finish stopping. If shutdown fails, updates remain blocked and **Retry maintenance** lets you retry after resolving the reported error.
...
4. Wait for the displayed phase and log. Only one maintenance job runs at a time; actions and resume are disabled while it is busy. Build commands have cancellation/timeouts; quitting Harbor cancels an active build and waits for its cleanup. A restore already switching component directories is allowed to finish, and shutdown waits for it rather than interrupting the switch halfway through.
...
Complete or cancel Diagnostics campaigns before maintenance. Entering maintenance pauses gateway requests, releases delivery workers and disconnects/stops upstream activity according to ownership; it does not itself cancel the separate diagnostic campaign. Previously running/starting entries are recorded for resumption. Server selections and project data are retained. Pausing new requests is distinct from completing shutdown: component operations require finished maintenance entry, and only one maintenance job runs at a time. A failed drain cannot authorize an update. Resume closes update admission immediately, then uses the current saved entries and startup decisions; a removed or unchecked entry should not be revived.
...
`pack` produces an unpacked app; `dist:win` produces the unsigned NSIS installer under the configured output directory, normally `release/`. Linux targets are available through `npm run dist:linux` and need target-specific GUI libraries and validation. The Windows renderer tests also require Microsoft Edge. Linux browser tests use `npx --no-install playwright install --with-deps chromium`; Electron tests need a graphical session or Xvfb. Optional installed-Hermes, installed-OpenClaw and tool-runtime tests use explicit environment opt-ins. The checked-in GitHub Actions workflow has Windows/Ubuntu test and packaging jobs; it is configuration, not evidence that those jobs have passed on the project repository.

**Phase 2 has an explicit staged build workflow.** The public `scripts/portable` inputs and helpers acquire pinned runtime, browser, package, source and embedding assets, verify their recorded identities, and build separate completed stages. `build-harbor-application.mjs` records an allowlisted application-source snapshot and builds the application; `assemble-portable.mjs` combines eleven pinned stage receipts into a fresh toolbox with a clean catalog and no copied user profile. Follow [Portable build inputs](portable-build-inputs.md) for the commands and their prerequisites. This is a multi-stage workflow, not a one-command bootstrap. Same-host offline builds and native candidate acceptance do not establish an independent clean Windows build or runtime result; those remain separate release gates. Dependency attribution and corresponding-source work also remain explicit. The supported end-user update path is the Portable bundle's **Maintenance** tab.
