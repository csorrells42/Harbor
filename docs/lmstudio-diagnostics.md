...
The controlled native API protocol campaign passed 27/27 cases with 62 actual Harbor tool dispatches: 12 conformance/curated trials, 10 representative file/SQLite/headless-browser tasks and five local search-delivery modes. Export and archive checks passed; internal model presentation correctly remained unknown. Native adapter and registration-ownership tests additionally cover malformed streams, argument errors, cancellation and preservation of other server entries. These establish adapter behavior, not model quality. Live checks on installed LM Studio 0.4.21+2 found the documented public-address restriction for ephemeral MCPs and a 403 on configured MCP use until the server permission is enabled. Those trials are excluded infrastructure failures, not Qwen failures. Successful installed native MCP execution remains an explicit acceptance gate until recorded.

The September 20 installed trial subsequently established authenticated native execution with Qwen3 0.6B Q8_0 at its unchanged 8,192-token context and one prediction slot. A no-tool trial passed. A lookup trial completed normally and made a real Harbor read; it omitted the required write while claiming completion, and the independent grader correctly marked the task incomplete. This verifies observable native execution and grading, not a capable model or a complete workload campaign. Earlier infrastructure failures remain preserved. Temporary tokens, registrations and server-setting changes were cleaned up afterward. These final source fixes postdate the Stage20/candidate12 archives and require a refreshed package.
