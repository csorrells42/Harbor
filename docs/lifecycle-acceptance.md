...
The preceding application source passed Windows/Linux CI at `c7f5ec7`. The later test-only commit `1609301` passed Linux CI but exposed two Windows managed-process failures: a child surviving into automatic restart and an endpoint still occupied on restart. Those failures remain recorded in [CI run 35500147894](https://github.com/csorrells42/Harbor/actions/runs/35500147894).

The subsequent runtime correction keeps an independent Windows guardian alive after supervisor termination. It waits on the supervisor's process handle, terminates its job, and acknowledges zero active job processes before Harbor considers cleanup complete. Missing acknowledgement is a failure, not permission to restart. Nineteen focused Windows checks passed locally, including normal restart, startup cancellation, command shims and [abrupt supervisor termination with a detached grandchild](../tests/core-managed-windows-crash.test.mjs). The same nineteen checks passed against packaged modules through the actual Electron runtime. A separate packaged-window check passed Start/Restart/Stop, reaped six tracked processes and displayed four gauges without renderer errors. [Windows/Linux CI for `f2c8c97`](https://github.com/csorrells42/Harbor/actions/runs/35501681851) and both distribution builds passed; the verified archive is installed locally. This functional change postdates the sealed security snapshot; no additional security scan was run. See [Phase 2 status](PHASE2-STATUS.md) for deployment boundaries.
